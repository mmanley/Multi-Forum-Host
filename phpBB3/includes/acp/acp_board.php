	...
	function main($id, $mode)
	{
		...
		*/
		...
		if ($submit)
		{
			...
			if ($mode == "settings") {
				$mfhclass->db->query("UPDATE `mfh_hosted_forums` SET `category_id` = '{$mfhclass->input->post_vars['forum_category']}' WHERE `access_name` = '{$mfhclass->info->forum_info['access_name']}';");
			}
		...
		while ($row = $mfhclass->db->fetch_array($sql)) {
			$forum_categories .= "<option value=\"{$row['category_id']}\" ".(($row['category_id'] == $mfhclass->info->forum_info['category_id']) ? "selected=\"selected\" " : NULL).">{$row['category_name']}</option>\n";
		}

		$template->assign_block_vars("host_options", array(
			"OPTIONS_ENABLED" => (($mode == "settings") ? true : false),
			"LEGEND_TITLE"    => "{$mfhclass->info->config['site_name']} settings",
			"OPTION_TITLE"    => "Forum Category",
			"OPTION_EXPLAIN"  => "The category you chose will determine where to place your forum within our <a href=\"../../../index.php?act=directory\" target=\"_blank\">directory</a>.",
			"OPTION_CONTENT"  => "<select name=\"forum_category\"><option type=\"-1\">None - Private Forum</option>{$forum_categories}</select>",
			"OPTION_NAME"     => "forum_category",
			)
		);
